...
有时候我们写外包的时候会遇到这么个情况，原系统已经有一套实体类了，但是因为某些原因又生成了一套实体类。那么我们应该怎么实现转化呢？

很多聪明的小伙伴肯定一下就想到了 `BeanUtils.copyProperties(source, target)`，但是呢，这个方法只能实现**属性名相同**的转化，如果属性名不同，就会出现 `null` 的情况（比如原实体类是下划线风格，新实体类是驼峰风格）。

::: warning
这种情况如果我们想要和 `BeanUtils.copyProperties()` 方法一样，那么我们就需要自定义转化方法，再封装为一个工具类。
:::
